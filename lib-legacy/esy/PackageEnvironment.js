...
function builtInsPerPackage(sandbox, prefix, packageInfo, installDirectory) {
  ...
  let name = _packageInfo$packageJ.name,
      ...
      dependencyTree = packageInfo.dependencyTree;
  ...
  return {
    ...
    [`${prefix}__depends`]: builtIn((0, (_keys || _load_keys()).default)(dependencyTree).join(' ')),
...
function addEnvConfigForPackage(_ref, realPathSandboxRootOnEjectingHost, packageName, packageJsonFilePath, exportedEnv) {
  let seenVars = _ref.seenVars,
      errors = _ref.errors,
      normalizedEnvVars = _ref.normalizedEnvVars;
...
function computeEnvVarsForPackage(sandbox, packageInfo) {
  let rootDirectory = packageInfo.rootDirectory,
  ...
  var _addEnvConfigForPacka = addEnvConfigForPackage({ seenVars: globalSeenVars, errors: errors, normalizedEnvVars: [] }, sandbox.packageInfo.rootDirectory, packageName, rootDirectory, autoExportedEnvVarsForPackage);

  let seenVars = _addEnvConfigForPacka.seenVars,
      nextErrors = _addEnvConfigForPacka.errors,
      normalizedEnvVars = _addEnvConfigForPacka.normalizedEnvVars;
  ...
  var _addEnvConfigForPacka2 = addEnvConfigForPackage({ seenVars: seenVars, errors: nextErrors, normalizedEnvVars: normalizedEnvVars }, sandbox.packageInfo.rootDirectory, packageName, path.join(rootDirectory, 'package.json'), packageJson.esy.exportedEnv);

  let nextSeenVars = _addEnvConfigForPacka2.seenVars,
      nextNextErrors = _addEnvConfigForPacka2.errors,
      nextNormalizedEnvVars = _addEnvConfigForPacka2.normalizedEnvVars;
...
function targetPath(sandbox, packageInfo, tree) {
  ...
  for (var _len = arguments.length, pathTo = Array(_len > 3 ? _len - 3 : 0), _key = 3; _key < _len; _key++) {
    pathTo[_key - 3] = arguments[_key];
  }

  if (isRootPackage) {
    return ['$esy__sandbox', tree].concat(pathTo).join('/');
  } else if (packageSourceType === 'local') {
    return ['$esy__local_store', tree, packageKey].concat(pathTo).join('/');
  }
  return ['$esy__store', tree, packageKey].concat(pathTo).join('/');
}
 ...
 */
function calculateEnvironment(sandbox, packageInfo) {
  ...
  function setUpBuiltinVariables(envConfigState) {
    let sandboxExportedEnvVars = (0, (_extends2 || _load_extends()).default)({
      'esy__sandbox': {
        val: '$ESY__SANDBOX',
        exclusive: true,
        __BUILT_IN_DO_NOT_USE_OR_YOU_WILL_BE_PIPd: true
    ...
    if (dependencies.length > 0) {
      ...
      sandboxExportedEnvVars = (0, (_assign || _load_assign()).default)(sandboxExportedEnvVars, {
        'PATH': {
          val: `${depPath}:$PATH`,
          exclusive: false
        },
  ...
  try {
    var _setUpBuiltinVariable = setUpBuiltinVariables({ seenVars: globalSeenVars, errors: [], normalizedEnvVars: [] });

    let seenVars = _setUpBuiltinVariable.seenVars,
        errors = _setUpBuiltinVariable.errors,
        normalizedEnvVars = _setUpBuiltinVariable.normalizedEnvVars;
